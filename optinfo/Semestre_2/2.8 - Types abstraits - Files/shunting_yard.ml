...
let shunting_yard infix_expression =
    ...
    and q = filevide () in
    let rec aux tokens op_stack =
       let rec search_opar stack =
              match stack with
              | EmptyStack -> stack
              | Node(o,st) when o = TKOpenPar -> st
              | Node(o,st) -> enfiler o q; search_opar st in
       let rec search_max_prec_op op stack =
               match stack with
              | EmptyStack -> stack
              | Node(o,st) when op_priority o > op_priority op -> enfiler o q; search_max_prec_op op st
              | Node(_,_) -> stack in
       match tokens with
        | [] -> op_stack
        | TKOpenPar::t -> print_string "("; aux t (push (TKOpenPar) op_stack)
        | TKClosingPar::t -> print_string ")"; let stack = search_opar op_stack in aux t stack
        | (TKNum n)::t -> print_int n; enfiler (TKNum n) q; aux t op_stack
        | (TKOp op)::t -> print_string op; let stack = search_max_prec_op (TKOp op) op_stack in aux t (push (TKOp op) stack)
...
let compute expr =
    ...
    let rec aux tk stack =
        match tk with
            ...
            | (TKOp op) :: t ->
               ( try let n2, st2 = pop stack in let n1, st =  pop st2 in
                match op with
                        | "+" -> aux t (push (n1 + n2) st)
                ...
                with
                | Failure _ -> failwith "Invalid RPN expression" )
