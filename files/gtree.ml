...
let bfs a =
  let rec explore file =
    match file with
    | [] -> []
    | Empty :: q -> explore q
    | Node (e, fils) :: q -> e :: explore (q @ fils)
  in
  explore [ a ]

let dfs a =
  let rec explore pile =
    match pile with
    | [] -> []
    | Empty :: q -> explore q
    | Node (e, fils) :: q -> e :: explore (fils @ q)
  in
  explore [ a ]
...
let rec convert_to_btree a =
  ...
  | Node (e, f :: t) ->
      let left = convert_to_btree f and right = convert_forest t in
      BNode (e, left, right)
  ...
  Node
    ( 'A',
      [
        ...
        Node
          ('I', [ Node ('J', [ Node ('K', []); Node ('L', []); Node ('M', []); Node ('N', []) ]) ]);
        Node ('O', []);
      ] )
;;
