...
let rec simple_quine_sat f =
  match simplify f with
      ...
      simple_quine_sat (subst v T sf) || simple_quine_sat (subst v F sf);;
...
let rec quine_sat f valuation =
  match simplify f with
      ...
      | F -> false
      | sf -> let v = max_var f 0 in quine_sat (subst v T sf) ((v,1)::valuation) || quine_sat (subst v F sf) ((v,0)::valuation);;
...
let f3 = And ((Var 0),  Or (Not (Var 0),(Var 2)));;
quine_sat f3;;
